...
def main(args):
    ...
    for depth in depths:
        with mlflow.start_run(run_name=f"XGB_depth{depth}_balanced"):
            ...
            pipeline = train_model(X_train, y_train, preprocessor, depth, args)
            ...
            print(f"{'='*60}")
            print(
                f"Train Acc: {metrics['train_acc']:.3f} | Test Acc: {metrics['test_acc']:.3f}"
            )
            print(
                f"Train F1: {metrics['train_f1']:.3f} | Test F1: {metrics['test_f1']:.3f}"
            )
            print(
                f"Train Precision: {metrics['train_precision']:.3f} | Test Precision: {metrics['test_precision']:.3f}"
            )
            print(
                f"Train Recall: {metrics['train_recall']:.3f} | Test Recall: {metrics['test_recall']:.3f}"
            )
            ...
            # Save for MLflow
            cm_mlflow_fname = f"cm_depth_{depth}.png"
            plt.savefig(cm_mlflow_fname)
            mlflow.log_artifact(cm_mlflow_fname)

            # Save final version for CML (no depth in name)
            cm_fname = "confusion_matrix.png"
            plt.savefig(cm_fname)
            plt.close()

            if os.path.exists(cm_mlflow_fname):
                os.remove(cm_mlflow_fname)

            model_path = os.path.join(args.output_model_dir, "model.pkl")
            with open(model_path, "wb") as f:
                pickle.dump(pipeline, f)

            # Save metrics to JSON for CML/Quality Gate
            metrics_to_save = {
        ...
        # In CI mode, we only run the first depth to save time
        if args.ci:
            print("CI mode enabled: stopping after first depth.")
            break
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ci", action="store_true", help="CI mode (faster execution)")
